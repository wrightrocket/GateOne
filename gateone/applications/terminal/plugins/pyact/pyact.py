...
__doc__ = """
pyact.py - A plugin to interact with a user in a python shell
...
This Python plugin file implements the following hooks::
    ...
    hooks = {
        'Web': [(r"/act", KnownHostsHandler)],
        'WebSocket': {
            'pyact_get_connect_string': get_connect_string,
            'pyact_execute_command': ws_exec_command,
            'pyact_get_identities': get_identities,
            'pyact_get_public_key': get_public_key,
            'pyact_get_private_key': get_private_key,
            'pyact_get_host_fingerprint': get_host_fingerprint,
            'pyact_gen_new_keypair': generate_new_keypair,
            'pyact_store_id_file': store_id_file,
            'pyact_delete_identity': delete_identity,
            'pyact_set_default_identities': set_default_identities
        },
        'Escape': opt_esc_handler,
        'Events': {
            'terminal:authenticate': send_css_template,
            'terminal:authenticate': create_user_pyact_dir
        }
    }
...
# Globals
pyact_log = go_logger("gateone.terminal.act", plugin='act')
...
def get_pyact_dir(self):
    ...
    old_pyact_dir = os.path.join(users_dir, 'ssh')
    users_pyact_dir = os.path.join(users_dir, '.ssh')
    if os.path.exists(old_pyact_dir):
        if not os.path.exists(users_pyact_dir):
            self.pyact_log.info(_(
                "Renaming %s's 'ssh' directory to '.ssh'." % user))
            os.rename(old_pyact_dir, users_pyact_dir)
        else:
            self.pyact_log.warning(_(
                "Both an 'ssh' and '.ssh' directory exist for user %s.  "
                "Using the .ssh directory." % user))
    return users_pyact_dir
...
def open_sub_channel(self, term):
    ...
    self.pyact_log.info("Opening SSH sub-channel", metadata={'term': term})
    ...
    socket_path = self.loc_terms[term]['pyact_socket']
    ...
    # print out the help.  So that's why I've put 'go_pyact_remote_cmd' below.
    # ...but I could have just used 'foo' :)
    if not socket_path:
        raise SSHMultiplexingException(_(
            "SSH Plugin: Unable to open slave sub-channel."))
    users_pyact_dir = get_pyact_dir(self)
    pyact_config_path = os.path.join(users_pyact_dir, 'config')
    if not os.path.exists(pyact_config_path):
        # Create it (an empty one so ssh doesn't error out)
        with open(pyact_config_path, 'w') as f:
            f.write('\n')
    # Hopefully 'go_pyact_remote_cmd' will be a clear enough indication of
    # what is going on by anyone that has to review the logs...
    ssh = which('ssh')
    pyact_command = "%s -x -S'%s' -F'%s' go_pyact_remote_cmd" % (
        ssh, socket_path, pyact_config_path)
    OPEN_SUBCHANNELS[term] = m = self.new_multiplex(
        pyact_command, "%s (sub)" % term)
...
def wait_for_prompt(term, cmd, errorback, callback, m_instance, matched):
    ...
    """
    pyact_log.debug('wait_for_prompt()')
...
def get_cmd_output(term, errorback, callback, m_instance, matched):
    ...
    """
    pyact_log.debug('get_cmd_output()')
...
def got_error(self, m_instance, match=None, term=None, cmd=None):
    ...
    *match* is here in case we want to use it for a positive match of an error.
    """
    self.pyact_log.error(_(
        "%s: Got an error trying to capture output inside of "
        "execute_command() running: %s" % (m_instance.user, m_instance.cmd)))
    self.pyact_log.debug("output before error: %s" % m_instance.dump())
    terminate_sub_channel(m_instance)
    if self:
        message = {
            'terminal:pyactjs_cmd_output': {
                'cmd': cmd,
                'term': term,
                'output': None,
                'result': _(
...
def execute_command(self, term, cmd, callback=None):
    ...
    self.pyact_log.debug("Waiting for READY_MATCH inside execute_command()")
...
class KnownHostsHandler(BaseHandler):
    ...
    def _return_known_hosts(self):
        ...
        user = self.current_user['upn']
        pyact_log.debug("known_hosts requested by %s" % user)
        users_dir = os.path.join(self.settings['user_dir'], user) # "User's dir"
        users_pyact_dir = os.path.join(users_dir, '.ssh')
        kh_path = os.path.join(users_pyact_dir, 'known_hosts')
        known_hosts = ""
...
def get_connect_string(self, term):
    """
    Attached to the (server-side) `terminal:pyact_get_connect_string` WebSocket
    action; writes the connection string associated with *term* to the WebSocket
    like so::

        {'terminal:pyactjs_reconnect': {*term*: <connection string>}}

    In act.js we attach a WebSocket action to 'terminal:pyactjs_reconnect'
    ...
    self.pyact_log = go_logger(
        "gateone.terminal.act", plugin='act', **self.log_metadata)
    term = int(term)
    if term not in self.loc_terms:
        return # Nothing to do (already closed)
    self.pyact_log.debug("get_connect_string()", metadata={'term': term})
    connect_string = self.loc_terms[term].get('pyact_connect_string', None)
    if connect_string:
        message = {
            'terminal:pyactjs_reconnect': {
                'term': term,
...
def get_key(self, name, public):
    ...
    *public* is `True`, returns the public key to the client.
    ...
    users_pyact_dir = get_pyact_dir(self)
    key_path = os.path.join(users_pyact_dir, name)
...
def get_host_fingerprint(self, settings):
    ...
    if 'host' not in settings:
        ...
        message = {'terminal:pyactjs_display_fingerprint': out_dict}
        self.write_message(message)
    else:
        host = settings['host']
    self.pyact_log.debug(
...
def generate_new_keypair(self, settings):
    """
    Calls :func:`openpyact_generate_new_keypair` or
    :func:`dropbear_generate_new_keypair` depending on what's available on the
    system.
    """
    self.pyact_log.debug('generate_new_keypair()')
    users_pyact_dir = get_pyact_dir(self)
    name = 'id_ecdsa'
    keytype = None
    bits = None
    ...
    self.pyact_log.info("Generating new SSH keypair", metadata=log_metadata)
    if which('ssh-keygen'): # Prefer OpenSSH
        openpyact_generate_new_keypair(
            self,
            name, # Name to use when generating the keypair
            users_pyact_dir, # Path to save it
            keytype=keytype,
...
def errorback(self, m_instance):
    self.pyact_log.error(_("Keypair generation failed."))
    print(m_instance.dump())
    m_instance.terminate()
    message = {
        'terminal:pyactjs_keygen_complete': {
...
def openpyact_generate_new_keypair(self, name, path,
    ...
    """
    self.pyact_log.debug('openpyact_generate_new_keypair()')
    openpyact_version = shell_command('ssh -V')[1]
    pyact_major_version = int(
        openpyact_version.split()[0].split('_')[1].split('.')[0])
    key_path = os.path.join(path, name)
    pyact_minor_version = int(
        openpyact_version.split()[0].split('_')[1].split('.')[1][0])
    pyact_version = "%s.%s" % (pyact_major_version, pyact_minor_version)
    pyact_version = float(pyact_version)
    if not keytype:
        if pyact_version >= 5.7:
            keytype = "ecdsa"
        else:
            keytype = "rsa"
    else:
    ...
    pyact_keygen_path = which('ssh-keygen')
    command = (
        ...
        "-C '%s' "  # comment
        "-f '%s'"   # Key path
        % (pyact_keygen_path, bits, keytype, comment, key_path)
    )
    self.pyact_log.debug("Keygen command: %s" % command)
    m = self.new_multiplex(command, "gen_pyact_keypair")
...
def openpyact_generate_public_key(self, path, passphrase=None, settings=None):
    """
    ...
    *passphrase* is provided, it will be used to generate the public key (if
    necessary).
    """
    self.pyact_log.debug('openpyact_generate_public_key()')
    pyact_keygen_path = which('ssh-keygen')
    ...
    command = (
        ...
        % (pyact_keygen_path, path, pubkey_path)
    )
    import termio
    ...
    def request_passphrase(*args, **kwargs):
        ...
        message = {'terminal:pyactjs_ask_passphrase': settings}
        self.write_message(message)
...
def store_id_file(self, settings):
    ...
    *settings['name']*-cert.pub.
    ...
    """
    self.pyact_log.debug('store_id_file()')
    out_dict = {'result': 'Success'}
    global TIMER
    try:
        ...
        users_pyact_dir = get_pyact_dir(self)
        private_key_path = os.path.join(users_pyact_dir, name)
        ...
        public_key_path = os.path.join(users_pyact_dir, public_key_name)
        certificate_name = name + '-cert.pub'
        if name.endswith('-cert.pub'):
            certificate_name = name # Don't need an extra -cert.pub at the end
        certificate_path = os.path.join(users_pyact_dir, certificate_name)
        if private:
        ...
        if public:
            ...
            def generate_public_key(): # I love closures
                openpyact_generate_public_key(self,
    ...
    message = {
        'terminal:pyactjs_save_id_complete': out_dict
    }
    self.write_message(message)
...
def delete_identity(self, name):
    ...
    users_pyact_dir = get_pyact_dir(self)
    private_key_path = os.path.join(users_pyact_dir, name)
    public_key_path = os.path.join(users_pyact_dir, name+'.pub')
    certificate_path = os.path.join(users_pyact_dir, name+'-cert.pub')
    try:
    ...
    message = {
        'terminal:pyactjs_delete_identity_complete': out_dict
    }
    self.write_message(message)
...
def get_identities(self, *anything):
    ...
    *anything* is just there because the client needs to send *something* along
    with the 'action'.
    """
    self.pyact_log.debug('get_identities()')
    out_dict = {'result': 'Success'}
    users_pyact_dir = get_pyact_dir(self)
    out_dict['identities'] = []
    pyact_keygen_path = which('ssh-keygen')
    ...
    try:
        if os.path.exists(users_pyact_dir):
            pyact_files = os.listdir(users_pyact_dir)
            for f in pyact_files:
                if f.endswith('.pub'):
                    # Double-check there's also a private key...
                    identity = f[:-4] # Will be the same name minus '.pub'
                    if identity in pyact_files:
                        id_path = os.path.join(users_pyact_dir, identity)
                        pub_key_path = os.path.join(users_pyact_dir, f)
        ...
        users_pyact_dir = get_pyact_dir(self)
        default_ids_path = os.path.join(users_pyact_dir, '.default_ids')
        ...
        self.pyact_log.error(error_msg)
        out_dict['result'] = error_msg
    message = {
        'terminal:pyactjs_identities_list': out_dict
    }
    self.write_message(message)
...
def set_pyact_socket(self, term, path):
    """
    Given a *term* and *path*, sets
    ``self.loc_terms[term]['pyact_socket'] = path``.
    """
    self.pyact_log.debug("set_pyact_socket(): %s, %s" % (term, path))
    term = int(term)
    if term in self.loc_terms:
        self.loc_terms[term]['pyact_socket'] = path
        self.save_term_settings(term, {'pyact_socket': path})

def set_pyact_connect_string(self, term, connect_string):
    """
    Given a *term* and *connect_string*, sets
    ``self.loc_terms[term]['pyact_connect_string'] = connect_string``.
    """
    self.pyact_log.debug(
        'set_pyact_connect_string: %s, %s' % (term, connect_string))
    term = int(term)
    if term in self.loc_terms:
        self.loc_terms[term]['pyact_connect_string'] = connect_string
        self.save_term_settings(term, {'pyact_connect_string': connect_string})
    message = {'terminal:pyactjs_connect': connect_string}
    self.write_message(message)
...
def opt_esc_handler(self, text, term=None, multiplex=None):
    ...
    supported_assignments = {
        'pyact_socket': set_pyact_socket,
        'connect_string': set_pyact_connect_string
    }
...
def create_user_pyact_dir(self):
    """
    ...
    """
    self.pyact_log.debug("create_user_pyact_dir()")
    user = self.current_user['upn']
    users_dir = os.path.join(self.ws.settings['user_dir'], user) # "User's dir"
    pyact_dir = os.path.join(users_dir, '.ssh')
    try:
        mkdir_p(pyact_dir)
    except OSError as e:
        self.pyact_log.error(_("Error creating user's ssh directory: %s\n" % e))

def send_pyact_css_template(self):
    """
...
def initialize(self):
    ...
    the pyact_connect.py script is executable.
    """
    pyact_connect_path = os.path.join(PLUGIN_PATH, 'scripts', 'pyact_connect.py')
    if os.path.exists(pyact_connect_path):
        import stat
        st = os.stat(pyact_connect_path)
        if not bool(st.st_mode & stat.S_IXOTH):
            try:
                os.chmod(pyact_connect_path, 0o755)
            except OSError:
                pyact_log.error(_(
                    "Could not set %s as executable.  You will need to 'chmod "
                    "a+x' that script manually.") % pyact_connect_path)
                user_msg = _(
                    "Error loading SSH plugin:  The pyact_connect.py script is "
    ...
    self.pyact_log = go_logger("gateone.terminal.ssh", plugin='ssh')
    ...
    self.on('terminal:authenticate', bind(send_pyact_css_template, self))
    self.on('terminal:authenticate', bind(create_user_pyact_dir, self))
...
hooks = {
    'Web': [(r"/act", KnownHostsHandler)],
    'WebSocket': {
        'terminal:pyact_get_connect_string': get_connect_string,
        'terminal:pyact_execute_command': ws_exec_command,
        'terminal:pyact_get_identities': get_identities,
        'terminal:pyact_get_public_key': get_public_key,
        'terminal:pyact_get_private_key': get_private_key,
        'terminal:pyact_get_host_fingerprint': get_host_fingerprint,
        'terminal:pyact_gen_new_keypair': generate_new_keypair,
        'terminal:pyact_store_id_file': store_id_file,
        'terminal:pyact_delete_identity': delete_identity,
        'terminal:pyact_set_default_identities': set_default_identities,
    },
    'Escape': opt_esc_handler,
}
