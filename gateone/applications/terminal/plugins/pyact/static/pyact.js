...
GateOne.Base.superSandbox("GateOne.ACT", ["GateOne.Bookmarks", "GateOne.Terminal", "GateOne.Terminal.Input", "GateOne.Editor"], function(window, undefined) {
"use strict";
...
// GateOne.ACT (ssh client functions)
go.Base.module(GateOne, "ACT", "0.1", ['Base']);
go.ACT.identities = []; // ACT identity objects end up in here
go.ACT.remoteCmdCallbacks = {};
go.ACT.remoteCmdErrorbacks = {};
go.noSavePrefs['autoConnectURL'] = null; // So it doesn't get saved in localStorage
go.Base.update(go.ACT, {
    init: function() {
        /**:GateOne.ACT.init()

        Creates the ACT Identity Manager panel, adds some buttons to the Info & Tools panel, and registers the following WebSocket actions & events::

            GateOne.Net.addAction('terminal:actjs_connect', GateOne.ACT.handleConnect);
            GateOne.Net.addAction('terminal:actjs_reconnect', GateOne.ACT.handleReconnect);
            GateOne.Net.addAction('terminal:actjs_keygen_complete', GateOne.ACT.keygenComplete);
            GateOne.Net.addAction('terminal:actjs_save_id_complete', GateOne.ACT.saveComplete);
            GateOne.Net.addAction('terminal:actjs_display_fingerprint', GateOne.ACT.displayHostFingerprint);
            GateOne.Net.addAction('terminal:actjs_identities_list', GateOne.ACT.incomingIDsAction);
            GateOne.Net.addAction('terminal:actjs_delete_identity_complete', GateOne.ACT.deleteCompleteAction);
            GateOne.Net.addAction('terminal:actjs_cmd_output', GateOne.ACT.commandCompleted);
            GateOne.Net.addAction('terminal:actjs_ask_passphrase', GateOne.ACT.enterPassphraseAction);
            GateOne.Events.on("terminal:new_terminal", GateOne.ACT.getConnectString);
        */
                    ...
                    if (u.startsWith('ssh://', str) || u.startsWith('telnet://', str)) {
                        var connect = function(term) {
                            ...
                            setTimeout(function() {
                                ...
                                if (go.Terminal.terminals[term]['title'] == gettext('ACT Connect')) {
                        ...
                        logError(gettext("ACT Plugin:  ssh query string must start with ssh:// or telnet:// (e.g. ssh=ssh://)"));
                    }
                } else {
        ...
        prefsPanelKnownHosts.onclick = function() {
            u.xhrGet(go.prefs.url+'ssh?known_hosts=True', go.ACT.updateKH);
        }
        ...
        infoPanelDuplicateSession.onclick = function() {
            ...
            go.ACT.duplicateSession(term);
        }
        h3.innerHTML = gettext("ACT Plugin");
        ...
        go.ACT.createPanel();
        go.Net.addAction('terminal:actjs_connect', go.ACT.handleConnect);
        go.Net.addAction('terminal:actjs_reconnect', go.ACT.handleReconnect);
        go.Net.addAction('terminal:actjs_keygen_complete', go.ACT.keygenComplete);
        go.Net.addAction('terminal:actjs_save_id_complete', go.ACT.saveComplete);
        go.Net.addAction('terminal:actjs_display_fingerprint', go.ACT.displayHostFingerprint);
        go.Net.addAction('terminal:actjs_identities_list', go.ACT.incomingIDsAction);
        go.Net.addAction('terminal:actjs_delete_identity_complete', go.ACT.deleteCompleteAction);
        go.Net.addAction('terminal:actjs_cmd_output', go.ACT.commandCompleted);
        go.Net.addAction('terminal:actjs_ask_passphrase', go.ACT.enterPassphraseAction);
        if (!go.prefs.broadcastTerminal) {
            E.on("terminal:new_terminal", go.ACT.autoConnect);
            E.on("terminal:new_terminal", go.ACT.getConnectString);
        }
        if (!go.prefs.embedded) {
            go.Input.registerShortcut('KEY_D', {'modifiers': {'ctrl': true, 'alt': true, 'meta': false, 'shift': false}, 'action': 'GateOne.ACT.duplicateSession(localStorage[GateOne.prefs.prefix+"selectedTerminal"])'});
        }
    },
    postInit: function() {
        /**:GateOne.ACT.postInit()
        ...
        */
        go.Bookmarks.registerURLHandler('ssh', go.ACT.connect);
        go.Bookmarks.registerIconHandler('ssh', go.ACT.bookmarkIconHandler);
        go.Bookmarks.registerURLHandler('telnet', go.ACT.connect);
        go.Bookmarks.registerIconHandler('telnet', go.ACT.bookmarkIconHandler);
    },
    bookmarkIconHandler: function(bookmark) {
        /**:GateOne.ACT.bookmarkIconHandler(bookmark)

        Saves the `GateOne.Icons.ACT` icon in the given bookmark using `GateOne.Bookmarks.storeFavicon()`.

        .. note:: This gets registered for the 'ssh' and 'telnet' inside of `GateOne.ACT.postInit()`.
        */
        go.Bookmarks.storeFavicon(bookmark, go.Icons['ssh']);
    },
    connect: function(URL) {
        /**:GateOne.ACT.connect(URL)

        Connects to the given ACT *URL*.

        If the current terminal is sitting at the ACT Connect prompt it will be used to make the connection.  Otherwise a new terminal will be opened.
        */
        logDebug("GateOne.ACT.connect: " + URL);
        var term = localStorage[prefix+'selectedTerminal'],
            unconnectedTermTitle = gettext('ACT Connect'), // NOTE: This MUST be equal to the title set by ssh_connect.py or it will send the ssh:// URL to the active terminal
    ...
    createPanel: function() {
        /**:GateOne.ACT.createPanel()

        Creates the ACT identity management panel (the shell of it anyway).
        */
        var ssh = go.ACT,
        ...
        sshIDHeaderH2.innerHTML = gettext('ACT Identity Manager: Loading...');
        ...
        certSpan.title = gettext("This field indicates whether or not there's an X.509 certificate associated with this identity (i.e. a '*name*-cert.pub' file).  X.509 certificates (for use with ACT) are created by signing a public key using a Certificate Authority (CA).  NOTE: In order to use X.509 certificates for authentication with ACT the servers you're connecting to must be configured to trust keys signed by a given CA.");
        commentSpan.innerHTML = gettext("Comment");
        commentSpan.title = gettext("This field will contain the comment from the identity's public key.  It comes after the key itself inside its .pub file and if the key was generated by OpenACT it will typically be something like, 'user@host'.");
        if (localStorage[prefix+'ssh_ids_sort'] == 'alpha') {
    ...
    loadIDs: function() {
        ...
        Toggles the ACT Identity Manager into view (if not already visible) and asks the server to send us our list of identities.
        */
        var ssh = go.ACT,
            existingPanel = u.getNode('#'+prefix+'panel_ssh_ids');
    ...
    incomingIDsAction: function(message) {
        /**:GateOne.ACT.incomingIDsAction(message)

        This gets attached to the 'actjs_identities_list' WebSocket action.  Adds *message['identities']* to `GateOne.ACT.identities` and places them into the Identity Manager.
        */
        var ssh = go.ACT,
            existingPanel = u.getNode('#'+prefix+'panel_ssh_ids'),
        ...
        ssh.delay = 500;
        sshIDHeaderH2.innerHTML = gettext("ACT Identity Manager");
    },
    displayMetadata: function(identity) {
        /**:GateOne.ACT.displayMetadata(identity)

        Displays the information about the given *identity* (its name) in the ACT identities metadata area (on the right).  Also displays the buttons that allow the user to delete the identity or upload a certificate.
        */
        var ssh = go.ACT,
    ...
    createIDItem: function(container, IDObj, delay) {
        /**:GateOne.ACT.displayMetadata(container, IDObj, delay)

        Creates an ACT identity element using *IDObj* and places it into *container*.
    ...
    getMaxIDs: function(elem) {
        /**:GateOne.ACT.getMaxIDs(elem)

        Calculates and returns the number of ACT identities that will fit in the given element ID (elem).
        */
        try {
            var ssh = go.ACT,
                node = u.getNode(elem),
    ...
    newIDForm: function() {
        /**:GateOne.ACT.newIDForm()

        Displays the dialog/form where the user can create or edit an ACT identity.
        */
        var ssh = go.ACT,
        ...
        var closeDialog = go.Visual.dialog(gettext('New ACT Identity'), identityForm, {'class': '✈prefsdialog', 'style': {'width': '20em'}}); // Just an initial width
    ...
    uploadIDForm: function() {
        /**:GateOne.ACT.uploadIDForm()

        Displays the dialog/form where a user can upload an ACT identity (that's already been created).
        */
        var ssh = go.ACT,
            goDiv = go.node,
            sshIDPanel = u.getNode('#'+prefix+'panel_ssh_ids'),
        ...
        var closeDialog = go.Visual.dialog(gettext('Upload ACT Identity'), uploadIDForm, {'class': '✈prefsdialog', 'style': {'width': '20em'}});
        cancel.onclick = closeDialog;
    ...
    uploadCertificateForm: function(identity) {
        /**:GateOne.ACT.uploadCertificateForm(identity)
    ...
    enterPassphraseAction: function(settings) {
        /**:GateOne.ACT.enterPassphraseAction(settings)
        ...
        */
        ...
        explanation.innerHTML = gettext("The private key for this ACT identity is protected by a passphrase.  Please enter the passphrase so a public key can be generated.");
    ...
    getConnectString: function(term) {
        /**:GateOne.ACT.getConnectString(term)

        Asks the ACT plugin on the Gate One server what the ACT connection string is for the given *term*.
        */
    ...
    deleteCompleteAction: function(message) {
        /**:GateOne.ACT.deleteCompleteAction(message)

        Called when an identity is deleted, calls :js:meth:`GateOne.ACT.loadIDs`
        */
        go.ACT.loadIDs();
    },
    handleConnect: function(connectString) {
        /**:GateOne.ACT.handleConnect(connectString)

        Handles the `terminal:actjs_connect` WebSocket action which should provide an ACT *connectString* in the form of 'user@host:port'.
        ...
        Also requests the host's public ACT key so it can be displayed to the user.
        */
    ...
    handleReconnect: function(message) {
        /**:GateOne.ACT.handleReconnect(message)

        Handles the `terminal:actjs_reconnect` WebSocket action which should provide an object containing each terminal's ACT connection string.  Example *message*::
    ...
    keygenComplete: function(message) {
        /**:GateOne.ACT.keygenComplete(message)
    ...
    saveComplete: function(message) {
        /**:GateOne.ACT.saveComplete(message)
    ...
    duplicateSession: function(term) {
        /**:GateOne.ACT.duplicateSession(term)

        Duplicates the ACT session at *term* in a new terminal.
        */
    ...
    updateKH: function(known_hosts) {
        /**:GateOne.ACT.updateKH(known_hosts)
    ...
    createKHPanel: function() {
        ...
        */
        ...
        sshHeader.innerHTML = '<h2>' + gettext('ACT Plugin: Edit Known Hosts') + '</h2>';
            ...
            var kh = u.getNode('#'+prefix+'ssh_kh_textarea').value,
                ...
                handleStateChange = function(e) {
                    ...
                    if (e.target.readyState == 4 && status == 200 && e.target.responseText) {
                        v.displayMessage(gettext("ACT Plugin: known_hosts saved."));
                        // Hide the panel
    ...
    displayHostFingerprint: function(message) {
        /**:GateOne.ACT.displayHostFingerprint(message)
    ...
    commandCompleted: function(message) {
        /**:GateOne.ACT.commandCompleted(message)

        Uses the contents of *message* to report the results of the command executed via :js:meth:`~GateOne.ACT.execRemoteCmd`.
        ...
        If a callback was registered in :js:attr:`GateOne.ACT.remoteCmdCallbacks[term]` it will be called like so::

            callback(message['output'])

        Otherwise the output will just be displayed to the user.  After the callback has executed it will be removed from `GateOne.ACT.remoteCmdCallbacks`.
        */
        var term = message['term'],
            cmd = message['cmd'],
        ...
        if (result != 'Success') {
            ...
            if (go.ACT.remoteCmdErrorbacks[term][cmd]) {
                go.ACT.remoteCmdErrorbacks[term][cmd](result);
                delete go.ACT.remoteCmdErrorbacks[term][cmd];
            }
            return;
        }
        if (go.ACT.remoteCmdCallbacks[term][cmd]) {
            go.ACT.remoteCmdCallbacks[term][cmd](output);
            delete go.ACT.remoteCmdCallbacks[term][cmd];
    ...
    execRemoteCmd: function(term, command, callback, errorback) {
        /**:GateOne.ACT.execRemoteCmd(term, command, callback, errorback)

        Executes *command* by creating a secondary shell in the background using the multiplexed tunnel of *term* (works just like :js:meth:`~GateOne.ACT.duplicateSession`).
...
// This is the favicon that gets used for ACT URLs in bookmarks
