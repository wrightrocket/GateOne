...
__doc__ = """
pyexp.py - A plugin to interact with a user in a python shell
...
This Python plugin file implements the following hooks::
    ...
    hooks = {
        'Web': [(r"/pyexp", KnownHostsHandler)],
        'WebSocket': {
            'pyexp_get_connect_string': get_connect_string,
            'pyexp_execute_command': ws_exec_command,
            'pyexp_get_identities': get_identities,
            'pyexp_get_public_key': get_public_key,
            'pyexp_get_private_key': get_private_key,
            'pyexp_get_host_fingerprint': get_host_fingerprint,
            'pyexp_gen_new_keypair': generate_new_keypair,
            'pyexp_store_id_file': store_id_file,
            'pyexp_delete_identity': delete_identity,
            'pyexp_set_default_identities': set_default_identities
        },
        'Escape': opt_esc_handler,
        'Events': {
            'terminal:authenticate': send_css_template,
            'terminal:authenticate': create_user_pyexp_dir
        }
    }
...
# Globals
pyexp_log = go_logger("gateone.terminal.pyexp", plugin='pyexp')
...
def get_pyexp_dir(self):
    ...
    old_pyexp_dir = os.path.join(users_dir, 'ssh')
    users_pyexp_dir = os.path.join(users_dir, '.ssh')
    if os.path.exists(old_pyexp_dir):
        if not os.path.exists(users_pyexp_dir):
            self.pyexp_log.info(_(
                "Renaming %s's 'ssh' directory to '.ssh'." % user))
            os.rename(old_pyexp_dir, users_pyexp_dir)
        else:
            self.pyexp_log.warning(_(
                "Both an 'ssh' and '.ssh' directory exist for user %s.  "
                "Using the .ssh directory." % user))
    return users_pyexp_dir
...
def open_sub_channel(self, term):
    ...
    self.pyexp_log.info("Opening SSH sub-channel", metadata={'term': term})
    ...
    socket_path = self.loc_terms[term]['pyexp_socket']
    ...
    # print out the help.  So that's why I've put 'go_pyexp_remote_cmd' below.
    # ...but I could have just used 'foo' :)
    if not socket_path:
        raise SSHMultiplexingException(_(
            "SSH Plugin: Unable to open slave sub-channel."))
    users_pyexp_dir = get_pyexp_dir(self)
    pyexp_config_path = os.path.join(users_pyexp_dir, 'config')
    if not os.path.exists(pyexp_config_path):
        # Create it (an empty one so ssh doesn't error out)
        with open(pyexp_config_path, 'w') as f:
            f.write('\n')
    # Hopefully 'go_pyexp_remote_cmd' will be a clear enough indication of
    # what is going on by anyone that has to review the logs...
    ssh = which('ssh')
    pyexp_command = "%s -x -S'%s' -F'%s' go_pyexp_remote_cmd" % (
        ssh, socket_path, pyexp_config_path)
    OPEN_SUBCHANNELS[term] = m = self.new_multiplex(
        pyexp_command, "%s (sub)" % term)
...
def wait_for_prompt(term, cmd, errorback, callback, m_instance, matched):
    ...
    """
    pyexp_log.debug('wait_for_prompt()')
...
def get_cmd_output(term, errorback, callback, m_instance, matched):
    ...
    """
    pyexp_log.debug('get_cmd_output()')
    ...
    # after a timeout (so we don't keep the sub-channel open forever).  It is
...
def got_error(self, m_instance, match=None, term=None, cmd=None):
    ...
    *match* is here in case we want to use it for a positive match of an error.
    """
    self.pyexp_log.error(_(
        "%s: Got an error trying to capture output inside of "
        "execute_command() running: %s" % (m_instance.user, m_instance.cmd)))
    self.pyexp_log.debug("output before error: %s" % m_instance.dump())
    terminate_sub_channel(m_instance)
    if self:
        message = {
            'terminal:pyexpjs_cmd_output': {
                'cmd': cmd,
                'term': term,
                'output': None,
                'result': _(
...
def execute_command(self, term, cmd, callback=None):
    ...
    self.pyexp_log.debug("Waiting for READY_MATCH inside execute_command()")
...
class KnownHostsHandler(BaseHandler):
    ...
    def _return_known_hosts(self):
        ...
        user = self.current_user['upn']
        pyexp_log.debug("known_hosts requested by %s" % user)
        users_dir = os.path.join(self.settings['user_dir'], user) # "User's dir"
        users_pyexp_dir = os.path.join(users_dir, '.ssh')
        kh_path = os.path.join(users_pyexp_dir, 'known_hosts')
        known_hosts = ""
...
def get_connect_string(self, term):
    """
    Attached to the (server-side) `terminal:pyexp_get_connect_string` WebSocket
    action; writes the connection string associated with *term* to the WebSocket
    like so::

        {'terminal:pyexpjs_reconnect': {*term*: <connection string>}}

    In act.js we attach a WebSocket action to 'terminal:pyexpjs_reconnect'
    ...
    self.pyexp_log = go_logger(
        "gateone.terminal.pyexp", plugin='pyexp', **self.log_metadata)
    term = int(term)
    if term not in self.loc_terms:
        return # Nothing to do (already closed)
    self.pyexp_log.debug("get_connect_string()", metadata={'term': term})
    connect_string = self.loc_terms[term].get('pyexp_connect_string', None)
    if connect_string:
        message = {
            'terminal:pyexpjs_reconnect': {
                'term': term,
...
def get_key(self, name, public):
    ...
    *public* is `True`, returns the public key to the client.
    ...
    users_pyexp_dir = get_pyexp_dir(self)
    key_path = os.path.join(users_pyexp_dir, name)
...
def get_host_fingerprint(self, settings):
    ...
    if 'host' not in settings:
        ...
        message = {'terminal:pyexpjs_display_fingerprint': out_dict}
        self.write_message(message)
    else:
        host = settings['host']
    self.pyexp_log.debug(
...
def generate_new_keypair(self, settings):
    """
    Calls :func:`openpyexp_generate_new_keypair` or
    :func:`dropbear_generate_new_keypair` depending on what's available on the
    system.
    """
    self.pyexp_log.debug('generate_new_keypair()')
    users_pyexp_dir = get_pyexp_dir(self)
    name = 'id_ecdsa'
    keytype = None
    bits = None
    ...
    self.pyexp_log.info("Generating new SSH keypair", metadata=log_metadata)
    if which('ssh-keygen'): # Prefer OpenSSH
        openpyexp_generate_new_keypair(
            self,
            name, # Name to use when generating the keypair
            users_pyexp_dir, # Path to save it
            keytype=keytype,
...
def errorback(self, m_instance):
    self.pyexp_log.error(_("Keypair generation failed."))
    print(m_instance.dump())
    m_instance.terminate()
    message = {
        'terminal:pyexpjs_keygen_complete': {
...
def openpyexp_generate_new_keypair(self, name, path,
    ...
    """
    self.pyexp_log.debug('openpyexp_generate_new_keypair()')
    openpyexp_version = shell_command('ssh -V')[1]
    pyexp_major_version = int(
        openpyexp_version.split()[0].split('_')[1].split('.')[0])
    key_path = os.path.join(path, name)
    pyexp_minor_version = int(
        openpyexp_version.split()[0].split('_')[1].split('.')[1][0])
    pyexp_version = "%s.%s" % (pyexp_major_version, pyexp_minor_version)
    pyexp_version = float(pyexp_version)
    if not keytype:
        if pyexp_version >= 5.7:
            keytype = "ecdsa"
        else:
            keytype = "rsa"
    else:
    ...
    pyexp_keygen_path = which('ssh-keygen')
    command = (
        ...
        "-C '%s' "  # comment
        "-f '%s'"   # Key path
        % (pyexp_keygen_path, bits, keytype, comment, key_path)
    )
    self.pyexp_log.debug("Keygen command: %s" % command)
    m = self.new_multiplex(command, "gen_pyexp_keypair")
...
def openpyexp_generate_public_key(self, path, passphrase=None, settings=None):
    """
    ...
    *passphrase* is provided, it will be used to generate the public key (if
    necessary).
    """
    self.pyexp_log.debug('openpyexp_generate_public_key()')
    pyexp_keygen_path = which('ssh-keygen')
    ...
    command = (
        ...
        % (pyexp_keygen_path, path, pubkey_path)
    )
    import termio
    ...
    def request_passphrase(*args, **kwargs):
        ...
        message = {'terminal:pyexpjs_ask_passphrase': settings}
        self.write_message(message)
...
def store_id_file(self, settings):
    ...
    *settings['name']*-cert.pub.
    ...
    """
    self.pyexp_log.debug('store_id_file()')
    out_dict = {'result': 'Success'}
    global TIMER
    try:
        ...
        users_pyexp_dir = get_pyexp_dir(self)
        private_key_path = os.path.join(users_pyexp_dir, name)
        ...
        public_key_path = os.path.join(users_pyexp_dir, public_key_name)
        certificate_name = name + '-cert.pub'
        if name.endswith('-cert.pub'):
            certificate_name = name # Don't need an extra -cert.pub at the end
        certificate_path = os.path.join(users_pyexp_dir, certificate_name)
        if private:
        ...
        if public:
            ...
            def generate_public_key(): # I love closures
                openpyexp_generate_public_key(self,
    ...
    message = {
        'terminal:pyexpjs_save_id_complete': out_dict
    }
    self.write_message(message)
...
def delete_identity(self, name):
    ...
    users_pyexp_dir = get_pyexp_dir(self)
    private_key_path = os.path.join(users_pyexp_dir, name)
    public_key_path = os.path.join(users_pyexp_dir, name+'.pub')
    certificate_path = os.path.join(users_pyexp_dir, name+'-cert.pub')
    try:
    ...
    message = {
        'terminal:pyexpjs_delete_identity_complete': out_dict
    }
    self.write_message(message)
...
def get_identities(self, *anything):
    ...
    *anything* is just there because the client needs to send *something* along
    with the 'action'.
    """
    self.pyexp_log.debug('get_identities()')
    out_dict = {'result': 'Success'}
    users_pyexp_dir = get_pyexp_dir(self)
    out_dict['identities'] = []
    pyexp_keygen_path = which('ssh-keygen')
    ...
    try:
        if os.path.exists(users_pyexp_dir):
            pyexp_files = os.listdir(users_pyexp_dir)
            for f in pyexp_files:
                if f.endswith('.pub'):
                    # Double-check there's also a private key...
                    identity = f[:-4] # Will be the same name minus '.pub'
                    if identity in pyexp_files:
                        id_path = os.path.join(users_pyexp_dir, identity)
                        pub_key_path = os.path.join(users_pyexp_dir, f)
        ...
        users_pyexp_dir = get_pyexp_dir(self)
        default_ids_path = os.path.join(users_pyexp_dir, '.default_ids')
        ...
        self.pyexp_log.error(error_msg)
        out_dict['result'] = error_msg
    message = {
        'terminal:pyexpjs_identities_list': out_dict
    }
    self.write_message(message)
...
def set_pyexp_socket(self, term, path):
    """
    Given a *term* and *path*, sets
    ``self.loc_terms[term]['pyexp_socket'] = path``.
    """
    self.pyexp_log.debug("set_pyexp_socket(): %s, %s" % (term, path))
    term = int(term)
    if term in self.loc_terms:
        self.loc_terms[term]['pyexp_socket'] = path
        self.save_term_settings(term, {'pyexp_socket': path})

def set_pyexp_connect_string(self, term, connect_string):
    """
    Given a *term* and *connect_string*, sets
    ``self.loc_terms[term]['pyexp_connect_string'] = connect_string``.
    """
    self.pyexp_log.debug(
        'set_pyexp_connect_string: %s, %s' % (term, connect_string))
    term = int(term)
    if term in self.loc_terms:
        self.loc_terms[term]['pyexp_connect_string'] = connect_string
        self.save_term_settings(term, {'pyexp_connect_string': connect_string})
    message = {'terminal:pyexpjs_connect': connect_string}
    self.write_message(message)
...
def opt_esc_handler(self, text, term=None, multiplex=None):
    ...
    use it to tell act.js what the SSH connection string is so it can use that
    ...
    supported_assignments = {
        'pyexp_socket': set_pyexp_socket,
        'connect_string': set_pyexp_connect_string
    }
...
def create_user_pyexp_dir(self):
    """
    ...
    """
    self.pyexp_log.debug("create_user_pyexp_dir()")
    user = self.current_user['upn']
    users_dir = os.path.join(self.ws.settings['user_dir'], user) # "User's dir"
    pyexp_dir = os.path.join(users_dir, '.ssh')
    try:
        mkdir_p(pyexp_dir)
    except OSError as e:
        self.pyexp_log.error(_("Error creating user's ssh directory: %s\n" % e))

def send_pyexp_css_template(self):
    """
    Sends our act.css template to the client using the 'load_style'
    WebSocket action.  The rendered template will be saved in Gate One's
    'cache_dir'.
    """
    css_path = os.path.join(PLUGIN_PATH, 'templates', 'pyact.css')
...
def initialize(self):
    ...
    the pyexp_connect.py script is executable.
    """
    pyexp_connect_path = os.path.join(PLUGIN_PATH, 'scripts', 'pyexp_connect.py')
    if os.path.exists(pyexp_connect_path):
        import stat
        st = os.stat(pyexp_connect_path)
        if not bool(st.st_mode & stat.S_IXOTH):
            try:
                os.chmod(pyexp_connect_path, 0o755)
            except OSError:
                pyexp_log.error(_(
                    "Could not set %s as executable.  You will need to 'chmod "
                    "a+x' that script manually.") % pyexp_connect_path)
                user_msg = _(
                    "Error loading SSH plugin:  The pyexp_connect.py script is "
    ...
    self.pyexp_log = go_logger("gateone.terminal.pyexp", plugin='pyexp')
    ...
    self.on('terminal:authenticate', bind(send_pyexp_css_template, self))
    self.on('terminal:authenticate', bind(create_user_pyexp_dir, self))
    # following lines are wishful thinking
    #user_msg = _("To start the python shell type: python")
    #send_msg = partial(self.ws.send_message, user_msg)
    #self.on('terminal:authenticate', send_msg)
    #execute_command(this.term, '/home/student/bin/expect2.py')

hooks = {
    'Web': [(r"/pyexp", KnownHostsHandler)],
    'WebSocket': {
        'terminal:pyexp_get_connect_string': get_connect_string,
        'terminal:pyexp_execute_command': ws_exec_command,
        'terminal:pyexp_get_identities': get_identities,
        'terminal:pyexp_get_public_key': get_public_key,
        'terminal:pyexp_get_private_key': get_private_key,
        'terminal:pyexp_get_host_fingerprint': get_host_fingerprint,
        'terminal:pyexp_gen_new_keypair': generate_new_keypair,
        'terminal:pyexp_store_id_file': store_id_file,
        'terminal:pyexp_delete_identity': delete_identity,
        'terminal:pyexp_set_default_identities': set_default_identities,
    },
    'Escape': opt_esc_handler,
}
