...
__doc__ = """\
pyact_connect.py - Opens an interactive python session with the given arguments and
...
# i18n support stuff
import gettext
gettext.bindtextdomain('pyact_connect', 'i18n')
gettext.textdomain('pyact_connect')
_ = gettext.gettext
...
def openssh_connect(
    ...
    if not script_path:
        ...
        temp = tempfile.NamedTemporaryFile(prefix="expect2.py", delete=False)
    ...
    #os.execvpe('/home/student/bin/pythonv01.py', args, env)
    #os.execvpe(action_script_path, args, env)
    #os.execvpe('/home/student/bin/expect2.py', args, env)
    os.execvpe(script_path, args, env)
...
def main():
    ...
    try:
        ...
        user = 'student'
        port = '22'
        password = None #'student'
        protocol = 'ssh://'
        validated = False
        debug = False
        # Set a pre-connection title
        print("\x1b]0;PYEXP Connect\007")
        if True:
            print(_('Connecting to ssh://%s@%s:%s' % (user, url, port)))
            # Set title
            print("\x1b]0;ssh://%s@%s\007" % (user, url))
            # Special escape handler (so the rest of the plugin knows the
            # connect string)
            connect_string = "{0}@{1}:{2}".format(user, url, port)
            print(
                "\x1b]_;ssh|set;connect_string;{0}\007".format(connect_string))
            openssh_connect(user, url, port,
                command=options.command,
                password=password,
                sshfp=options.sshfp,
                randomart=options.randomart,
...
if __name__ == "__main__":
    #import sys
    #import os
    #action_script_path = sys.argv[1]
    #if not os.path.isfile(action_script_path):
    #    os._exit(1)
