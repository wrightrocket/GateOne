...
__doc__ = """
act.py - A plugin to interact with a user in a terminal
...
This Python plugin file implements the following hooks::
    ...
    hooks = {
        ...
        'WebSocket': {
            'act_get_connect_string': get_connect_string,
            'act_execute_command': ws_exec_command,
            'act_get_identities': get_identities,
            'act_get_public_key': get_public_key,
            'act_get_private_key': get_private_key,
            'act_get_host_fingerprint': get_host_fingerprint,
            'act_gen_new_keypair': generate_new_keypair,
            'act_store_id_file': store_id_file,
            'act_delete_identity': delete_identity,
            'act_set_default_identities': set_default_identities
        },
        'Escape': opt_esc_handler,
        'Events': {
            'terminal:authenticate': send_css_template,
            'terminal:authenticate': create_user_act_dir
        }
    }
...
# Globals
act_log = go_logger("gateone.terminal.act", plugin='act')
...
def get_act_dir(self):
    ...
    old_act_dir = os.path.join(users_dir, 'ssh')
    users_act_dir = os.path.join(users_dir, '.ssh')
    if os.path.exists(old_act_dir):
        if not os.path.exists(users_act_dir):
            self.act_log.info(_(
                "Renaming %s's 'ssh' directory to '.ssh'." % user))
            os.rename(old_act_dir, users_act_dir)
        else:
            self.act_log.warning(_(
                "Both an 'ssh' and '.ssh' directory exist for user %s.  "
                "Using the .ssh directory." % user))
    return users_act_dir
...
def open_sub_channel(self, term):
    ...
    self.act_log.info("Opening SSH sub-channel", metadata={'term': term})
    ...
    socket_path = self.loc_terms[term]['act_socket']
    ...
    # print out the help.  So that's why I've put 'go_act_remote_cmd' below.
    # ...but I could have just used 'foo' :)
    if not socket_path:
        raise SSHMultiplexingException(_(
            "SSH Plugin: Unable to open slave sub-channel."))
    users_act_dir = get_act_dir(self)
    act_config_path = os.path.join(users_act_dir, 'config')
    if not os.path.exists(act_config_path):
        # Create it (an empty one so ssh doesn't error out)
        with open(act_config_path, 'w') as f:
            f.write('\n')
    # Hopefully 'go_act_remote_cmd' will be a clear enough indication of
    # what is going on by anyone that has to review the logs...
    ssh = which('ssh')
    act_command = "%s -x -S'%s' -F'%s' go_act_remote_cmd" % (
        ssh, socket_path, act_config_path)
    OPEN_SUBCHANNELS[term] = m = self.new_multiplex(
        act_command, "%s (sub)" % term)
...
def execute_command(self, term, cmd, callback=None):
    ...
    self.act_log.debug("Waiting for READY_MATCH inside execute_command()")
...
class KnownHostsHandler(BaseHandler):
    ...
    def _return_known_hosts(self):
        ...
        user = self.current_user['upn']
        act_log.debug("known_hosts requested by %s" % user)
        users_dir = os.path.join(self.settings['user_dir'], user) # "User's dir"
        users_act_dir = os.path.join(users_dir, '.ssh')
        kh_path = os.path.join(users_act_dir, 'known_hosts')
        known_hosts = ""
...
def get_connect_string(self, term):
    """
    Attached to the (server-side) `terminal:act_get_connect_string` WebSocket
    action; writes the connection string associated with *term* to the WebSocket
    like so::

        {'terminal:actjs_reconnect': {*term*: <connection string>}}

    In act.js we attach a WebSocket action to 'terminal:actjs_reconnect'
    ...
    self.act_log = go_logger(
        "gateone.terminal.act", plugin='act', **self.log_metadata)
    term = int(term)
    if term not in self.loc_terms:
        return # Nothing to do (already closed)
    self.act_log.debug("get_connect_string()", metadata={'term': term})
    connect_string = self.loc_terms[term].get('act_connect_string', None)
    if connect_string:
        message = {
            'terminal:actjs_reconnect': {
                'term': term,
...
def get_key(self, name, public):
    ...
    *public* is `True`, returns the public key to the client.
    ...
    users_act_dir = get_act_dir(self)
    key_path = os.path.join(users_act_dir, name)
...
def get_host_fingerprint(self, settings):
    ...
    if 'host' not in settings:
        ...
        message = {'terminal:actjs_display_fingerprint': out_dict}
        self.write_message(message)
    else:
        host = settings['host']
    self.act_log.debug(
...
def generate_new_keypair(self, settings):
    """
    Calls :func:`openact_generate_new_keypair` or
    :func:`dropbear_generate_new_keypair` depending on what's available on the
    system.
    """
    self.act_log.debug('generate_new_keypair()')
    users_act_dir = get_act_dir(self)
    name = 'id_ecdsa'
    keytype = None
    bits = None
    ...
    self.act_log.info("Generating new SSH keypair", metadata=log_metadata)
    if which('ssh-keygen'): # Prefer OpenSSH
        openact_generate_new_keypair(
            self,
            name, # Name to use when generating the keypair
            users_act_dir, # Path to save it
            keytype=keytype,
...
def errorback(self, m_instance):
    self.act_log.error(_("Keypair generation failed."))
    print(m_instance.dump())
    m_instance.terminate()
    message = {
        'terminal:actjs_keygen_complete': {
...
def openact_generate_new_keypair(self, name, path,
    ...
    """
    self.act_log.debug('openact_generate_new_keypair()')
    openact_version = shell_command('ssh -V')[1]
    act_major_version = int(
        openact_version.split()[0].split('_')[1].split('.')[0])
    key_path = os.path.join(path, name)
    act_minor_version = int(
        openact_version.split()[0].split('_')[1].split('.')[1][0])
    act_version = "%s.%s" % (act_major_version, act_minor_version)
    act_version = float(act_version)
    if not keytype:
        if act_version >= 5.7:
            keytype = "ecdsa"
        else:
    ...
    act_keygen_path = which('ssh-keygen')
    command = (
        ...
        "-C '%s' "  # comment
        "-f '%s'"   # Key path
        % (act_keygen_path, bits, keytype, comment, key_path)
    )
    self.act_log.debug("Keygen command: %s" % command)
    m = self.new_multiplex(command, "gen_act_keypair")
...
def openact_generate_public_key(self, path, passphrase=None, settings=None):
    """
    ...
    *passphrase* is provided, it will be used to generate the public key (if
    necessary).
    """
    self.act_log.debug('openact_generate_public_key()')
    act_keygen_path = which('ssh-keygen')
    ...
    command = (
        ...
        % (act_keygen_path, path, pubkey_path)
    )
    import termio
    ...
    def request_passphrase(*args, **kwargs):
        ...
        message = {'terminal:actjs_ask_passphrase': settings}
        self.write_message(message)
...
def store_id_file(self, settings):
    ...
    *settings['name']*-cert.pub.
    ...
    """
    self.act_log.debug('store_id_file()')
    out_dict = {'result': 'Success'}
    global TIMER
    try:
        ...
        users_act_dir = get_act_dir(self)
        private_key_path = os.path.join(users_act_dir, name)
        ...
        public_key_path = os.path.join(users_act_dir, public_key_name)
        certificate_name = name + '-cert.pub'
        if name.endswith('-cert.pub'):
            certificate_name = name # Don't need an extra -cert.pub at the end
        certificate_path = os.path.join(users_act_dir, certificate_name)
        if private:
        ...
        if public:
            ...
            def generate_public_key(): # I love closures
                openact_generate_public_key(self,
    ...
    message = {
        'terminal:actjs_save_id_complete': out_dict
    }
    self.write_message(message)
...
def delete_identity(self, name):
    ...
    users_act_dir = get_act_dir(self)
    private_key_path = os.path.join(users_act_dir, name)
    public_key_path = os.path.join(users_act_dir, name+'.pub')
    certificate_path = os.path.join(users_act_dir, name+'-cert.pub')
    try:
    ...
    message = {
        'terminal:actjs_delete_identity_complete': out_dict
    }
    self.write_message(message)
...
def get_identities(self, *anything):
    ...
    *anything* is just there because the client needs to send *something* along
    with the 'action'.
    """
    self.act_log.debug('get_identities()')
    out_dict = {'result': 'Success'}
    users_act_dir = get_act_dir(self)
    out_dict['identities'] = []
    act_keygen_path = which('ssh-keygen')
    ...
    try:
        if os.path.exists(users_act_dir):
            act_files = os.listdir(users_act_dir)
            for f in act_files:
                if f.endswith('.pub'):
                    # Double-check there's also a private key...
                    identity = f[:-4] # Will be the same name minus '.pub'
                    if identity in act_files:
                        id_path = os.path.join(users_act_dir, identity)
                        pub_key_path = os.path.join(users_act_dir, f)
        ...
        users_act_dir = get_act_dir(self)
        default_ids_path = os.path.join(users_act_dir, '.default_ids')
        ...
        self.act_log.error(error_msg)
        out_dict['result'] = error_msg
    message = {
        'terminal:actjs_identities_list': out_dict
    }
    self.write_message(message)
...
def set_act_socket(self, term, path):
    """
    Given a *term* and *path*, sets
    ``self.loc_terms[term]['act_socket'] = path``.
    """
    self.act_log.debug("set_act_socket(): %s, %s" % (term, path))
    term = int(term)
    if term in self.loc_terms:
        self.loc_terms[term]['act_socket'] = path
        self.save_term_settings(term, {'act_socket': path})

def set_act_connect_string(self, term, connect_string):
    """
    Given a *term* and *connect_string*, sets
    ``self.loc_terms[term]['act_connect_string'] = connect_string``.
    """
    self.act_log.debug(
        'set_act_connect_string: %s, %s' % (term, connect_string))
    term = int(term)
    if term in self.loc_terms:
        self.loc_terms[term]['act_connect_string'] = connect_string
        self.save_term_settings(term, {'act_connect_string': connect_string})
    message = {'terminal:actjs_connect': connect_string}
    self.write_message(message)
...
def opt_esc_handler(self, text, term=None, multiplex=None):
    ...
    supported_assignments = {
        'act_socket': set_act_socket,
        'connect_string': set_act_connect_string
    }
...
def create_user_act_dir(self):
    ...
    """
    self.act_log.debug("create_user_act_dir()")
    user = self.current_user['upn']
    users_dir = os.path.join(self.ws.settings['user_dir'], user) # "User's dir"
    act_dir = os.path.join(users_dir, '.ssh')
    try:
        mkdir_p(act_dir)
    except OSError as e:
        self.act_log.error(_("Error creating user's ssh directory: %s\n" % e))

def send_act_css_template(self):
    """
    ...
    'cache_dir'.
    """
    css_path = os.path.join(PLUGIN_PATH, 'templates', 'act.css')
...
def initialize(self):
    ...
    the act_connect.py script is executable.
    """
    act_connect_path = os.path.join(PLUGIN_PATH, 'scripts', 'act_connect.py')
    if os.path.exists(act_connect_path):
        import stat
        st = os.stat(act_connect_path)
        if not bool(st.st_mode & stat.S_IXOTH):
            try:
                os.chmod(act_connect_path, 0o755)
            except OSError:
                act_log.error(_(
                    "Could not set %s as executable.  You will need to 'chmod "
                    "a+x' that script manually.") % act_connect_path)
                user_msg = _(
                    "Error loading SSH plugin:  The act_connect.py script is "
    ...
    self.act_log = go_logger("gateone.terminal.ssh", plugin='ssh')
    ...
    self.on('terminal:authenticate', bind(send_act_css_template, self))
    self.on('terminal:authenticate', bind(create_user_act_dir, self))

hooks = {
    'Web': [(r"/act", KnownHostsHandler)],
    'WebSocket': {
        'terminal:act_get_connect_string': get_connect_string,
        'terminal:act_execute_command': ws_exec_command,
        'terminal:act_get_identities': get_identities,
        'terminal:act_get_public_key': get_public_key,
        'terminal:act_get_private_key': get_private_key,
        'terminal:act_get_host_fingerprint': get_host_fingerprint,
        'terminal:act_gen_new_keypair': generate_new_keypair,
        'terminal:act_store_id_file': store_id_file,
        'terminal:act_delete_identity': delete_identity,
        'terminal:act_set_default_identities': set_default_identities,
    },
    'Escape': opt_esc_handler,
}
